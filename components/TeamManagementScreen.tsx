...
const MOCK_TEAM: User[] = [
    { id: 1, name: "Femi (You)", contact: "femi@agripulse.com", role: "Owner", status: "Active" },
    { id: 2, name: "Aisha Bello", contact: "aisha.b@email.com", role: "Manager", status: "Active" },
    { id: 3, name: "Tunde Adebayo", contact: "+2348012345678", role: "Worker", status: "Active" },
    { id: 4, name: "Bola Johnson", contact: "bola@example.com", role: "Worker", status: "Pending" },
];
...
const InviteMemberModal: React.FC<InviteMemberModalProps> = ({ onClose, onInvite }) => {
    ...
    const RoleSelector: React.FC<{value: Role, label: string, description: string}> = ({ value, label, description}) => (
        <label className={`flex items-start p-3 border-2 rounded-xl cursor-pointer transition-all ${role === value ? 'border-primary bg-green-50 dark:bg-green-900/20' : 'border-border bg-card'}`}>
            <input type="radio" name="role" value={value} checked={role === value} onChange={() => setRole(value)} className="mt-1 mr-3 h-4 w-4 text-primary focus:ring-primary border-gray-300 dark:border-gray-600 dark:bg-gray-700"/>
            <div>
                <p className={`font-bold ${role === value ? 'text-primary' : 'text-text-primary'}`}>{label}</p>
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-30 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-popover rounded-2xl shadow-lg p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
                <h3 className="text-xl font-bold mb-4 text-text-primary">Invite New Team Member</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-text-secondary mb-1">Full Name *</label>
                        <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Enter their full name" className="w-full p-3 border border-border rounded-lg bg-card text-text-primary" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-text-secondary mb-1">Invite via *</label>
                        <div className="flex bg-muted p-1 rounded-lg">
                            <button type="button" onClick={() => setInviteMethod('email')} className={`flex-1 py-2 text-sm font-bold rounded-md transition ${inviteMethod === 'email' ? 'bg-card shadow text-primary' : 'text-text-secondary'}`}>Email</button>
                            <button type="button" onClick={() => setInviteMethod('phone')} className={`flex-1 py-2 text-sm font-bold rounded-md transition ${inviteMethod === 'phone' ? 'bg-card shadow text-primary' : 'text-text-secondary'}`}>Phone Number</button>
                        </div>
                    </div>
                    {inviteMethod === 'email' ? (
                        <div><input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="name@example.com" className="w-full p-3 border border-border rounded-lg bg-card text-text-primary" required /></div>
                    ) : (
                        <div><input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+234 801 234 5678" className="w-full p-3 border border-border rounded-lg bg-card text-text-primary" required /></div>
                    )}
                     <div>
                        <label className="block text-sm font-medium text-text-secondary mb-2">Assign Role *</label>
                        <div className="space-y-2">
                           <RoleSelector value="Manager" label="Farm Manager" description="Can view analytics and manage batches." />
                           <RoleSelector value="Worker" label="Farm Worker" description="Can only enter daily logs." />
                        </div>
                    </div>
                     <div className="flex justify-end gap-3 mt-4">
                        ...
                        <button type="submit" className="px-4 py-2 rounded-lg text-white bg-primary hover:bg-primary-600 font-semibold">Send Invite</button>
                    </div>
                </form>
            </div>
        </div>
