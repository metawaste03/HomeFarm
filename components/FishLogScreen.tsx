...
const FishLogScreen: React.FC<FishLogScreenProps> = ({ onNavigate, farm, batch }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (feedKg > 0 && selectedFeedId) {
                await recordUsage(selectedFeedId, feedKg, `Daily log (Fish) - ${date}`);
            }
    ...
    const hasInteracted = useMemo(() => {
        ...
        return feedKg > 0 || mortality > 0 || notes.trim().length > 0 || selectedFeedId !== '' || sampledfish > 0 || totalWeight > 0 || hasWaterQuality;
    }, [feedKg, mortality, notes, selectedFeedId, sampledfish, totalWeight, waterQuality]);
